...
class ModelTrainer:
    ...
    def prepare_data(self, code_samples: List[str], labels: List[int]) -> Tuple[List[str], List[int]]:
        """
    ...
    def extract_features(self, code_samples: List[str]) -> List[Dict]:
        ...
        for code in code_samples:
            ...
            try:
                ...
                feature_dict["num_functions"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.FunctionDef))
                feature_dict["num_classes"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.ClassDef))
                feature_dict["num_imports"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.Import) or isinstance(_, ast.ImportFrom))
                feature_dict["num_loops"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.For) or isinstance(_, ast.While))
                feature_dict["num_branches"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.If) or isinstance(_, ast.IfExp))
                feature_dict["num_except"] = sum(1 for _ in ast.walk(tree) if isinstance(_, ast.ExceptHandler))
